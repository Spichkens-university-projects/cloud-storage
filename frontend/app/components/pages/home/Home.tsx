...
const Home: FC<PropsWithChildren<Props>> = ({ children }) => {
    ...
    const { data: currentDirectoryFiles } = api.useGetFilesFromDirectoryQuery(
        dirId,
        { skip: !dirId },
    );

    const [createNewDirectory, { data, error, isError, isSuccess }] =
        api.useCreateNewDirectoryMutation();
    ...
    return (
        <Layout>
            ...
            <div className={'text-center text-5xl place-self-center'}>
                {!files && !currentDirectoryFiles ? 'Хранилище пусто' : null}
            </div>
        </Layout>
    );
};
